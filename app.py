...
app.config['SQLALCHEMY_DATABASE_URI']=f"postgresql://{DATABASE_USERNAME}:{DATABASE_PASSWORD}@{DATABASE_HOST}/{DATABASE}"
...
try:
    ...
    def add_post(city_id):
        if request.method == 'POST':
            data = request.json
            print(data)
            new_post = Post(city_code = data["city_code"], zip_code = data["zip_code"], available_date = data["available_date"], contact_email = data["contact_email"], image = data["image"], twenty_hookup = data["twenty_hookup"], thirty_hookup = data["thirty_hookup"], fifty_hookup = data["fifty_hookup"], wifi = data["wifi"], water = data["water"])
    ...
    def update_by_post_id(city_id, post_id):
        body = request.json
        post = Post.query.filter_by(post_id = post_id).update(body)
        print(post)
        db.session.commit()
